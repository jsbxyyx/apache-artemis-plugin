 ...
 * events.
 */
public class MQTT2ConnectionManager {

    private static final Logger log = LoggerFactory.getLogger(MQTT2ConnectionManager.class);
    private MQTT2Session session;

   public MQTT2ConnectionManager(MQTT2Session session) {
      this.session = session;
      MQTT2FailureListener failureListener = new MQTT2FailureListener(this);
   ...
   synchronized void connect(MqttConnectMessage connect, String validatedUser, String username, String password) throws Exception {
      if (session.getVersion() == MQTTVersion.MQTT_5) {
         ...
         String authenticationMethod = MQTT2Util.getProperty(String.class, connect.variableHeader().properties(), AUTHENTICATION_METHOD);
      ...
      MQTT2SessionState sessionState = session.getStateManager().getSessionState(clientId);
      ...
      if (session.getVersion() == MQTTVersion.MQTT_5) {
         session.getConnection().setReceiveMaximum(MQTT2Util.getProperty(Integer.class, connect.variableHeader().properties(), RECEIVE_MAXIMUM, -1));

         sessionState.setClientSessionExpiryInterval(MQTT2Util.getProperty(Integer.class, connect.variableHeader().properties(), SESSION_EXPIRY_INTERVAL, 0));
         sessionState.setClientMaxPacketSize(MQTT2Util.getProperty(Integer.class, connect.variableHeader().properties(), MAXIMUM_PACKET_SIZE, 0));
         sessionState.setClientTopicAliasMaximum(MQTT2Util.getProperty(Integer.class, connect.variableHeader().properties(), TOPIC_ALIAS_MAXIMUM));
   ...
   ServerSessionImpl createServerSession(String username, String password, String validatedUser) throws Exception {
      ...
      ServerSession serverSession = server.createSession(id,
                                                         ...
                                                         MQTT2Util.SESSION_AUTO_COMMIT_SENDS,
                                                         MQTT2Util.SESSION_AUTO_COMMIT_ACKS,
                                                         MQTT2Util.SESSION_PREACKNOWLEDGE,
                                                         MQTT2Util.SESSION_XA,
                                                         null,
                                                         session.getSessionCallback(),
                                                         MQTT2Util.SESSION_AUTO_CREATE_QUEUE,
   ...
   synchronized void disconnect(boolean failure) {
      ...
      try {
         ...
         MQTT2Logger.LOGGER.errorDisconnectingClient(e);
      } finally {
