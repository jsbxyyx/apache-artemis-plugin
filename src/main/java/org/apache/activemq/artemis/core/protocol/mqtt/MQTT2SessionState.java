...
public class MQTT2SessionState {

   private static final Logger logger = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());

   public static final MQTT2SessionState DEFAULT = new MQTT2SessionState((String) null);
   ...
   public MQTT2SessionState(String clientId) {
    ...
    */
   public MQTT2SessionState(CoreMessage message) {
   ...
   public MQTT2Session getSession() {
      return session;
   }

   public void setSession(MQTT2Session session) {
   ...
   public boolean addSubscription(MqttTopicSubscription subscription, WildcardConfiguration wildcardConfiguration, Integer subscriptionIdentifier) throws Exception {
      ...
      synchronized (subscriptions) {
         addressMessageMap.putIfAbsent(MQTT2Util.getCoreAddressFromMqttTopic(subscription.topicName(), wildcardConfiguration), new ConcurrentHashMap<>());
   ...
   public List<Integer> getMatchingSubscriptionIdentifiers(String address) {
      address = MQTT2Util.getMqttTopicFromCoreAddress(address, session.getServer().getConfiguration().getWildcardConfiguration());
      List<Integer> result = null;
      for (Pair<MqttTopicSubscription, Integer> pair : subscriptions.values()) {
         Pattern pattern = Match.createPattern(pair.getA().topicName(), MQTT2Util.MQTT_WILDCARD, true);
   ...
   public class OutboundStore {
      ...
      public int generateMqttId(long messageId, long consumerId) {
         synchronized (dataStoreLock) {
            ...
            if (id == null) {
               do {
                  if (currentId == MQTT2Util.TWO_BYTE_INT_MAX) {
                     currentId = 0;
                  }
                  ++currentId;
               }
