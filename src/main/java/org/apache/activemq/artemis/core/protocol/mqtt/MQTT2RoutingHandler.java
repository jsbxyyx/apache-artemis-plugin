...
public class MQTT2RoutingHandler extends RoutingHandler<MQTT2RoutingContext> {

   protected MQTT2RoutingHandler(ActiveMQServer server) {
      super(server);
   }

   public boolean route(MQTT2Connection mqttConnection, MQTT2Session mqttSession, MqttConnectMessage connect) throws Exception {
      return route(new MQTT2RoutingContext(mqttConnection, mqttSession, connect));
   }

   @Override
   protected void refuse(MQTT2RoutingContext context) {
   ...
   @Override
   protected void redirect(MQTT2RoutingContext context) {
