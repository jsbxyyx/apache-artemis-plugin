...
public class MQTT2Util {
     ...
     * {@link MQTT2Util#MQTT_RETAIN_ADDRESS_PREFIX}
     *
     * @param topicFilter           the MQTT topic filter
     * @param wildcardConfiguration the {@code WildcardConfiguration} governing the core broker
     * @return the name of the core address based on the input, stripping
     * {@link MQTT2Util#MQTT_RETAIN_ADDRESS_PREFIX} if it exists
     */
    ...
    private static ICoreMessage createServerMessage(MQTT2Session session, SimpleString address, MqttPublishMessage mqttPublishMessage) {
    ...
    public static Message createServerMessageFromByteBuf(MQTT2Session session,
    ...
    public static Message createPubRelMessage(MQTT2Session session, SimpleString address, int messageId) {
        ...
        Message message = createServerMessage(session, address, publishMessage)
                .putIntProperty(MQTT2Util.MQTT_MESSAGE_ID_KEY, messageId)
                .putIntProperty(MQTT2Util.MQTT_MESSAGE_TYPE_KEY, MqttMessageType.PUBREL.value());
        return message;
    }

    public static void logMessage(MQTT2SessionState state, MqttMessage message, boolean inbound, MQTTVersion version) {
        ...
        if (properties != null) {
            ...
            if (o != null) {
                try {
                    ...
                    MQTT2Logger.LOGGER.failedToCastProperty(propertyName.toString());
                    throw e;
                }
            }
        }
