...
import static org.apache.activemq.artemis.core.protocol.mqtt.MQTT2Util.MQTT_CONTENT_TYPE_KEY;
import static org.apache.activemq.artemis.core.protocol.mqtt.MQTT2Util.MQTT_CORRELATION_DATA_KEY;
import static org.apache.activemq.artemis.core.protocol.mqtt.MQTT2Util.MQTT_MESSAGE_RETAIN_INITIAL_DISTRIBUTION_KEY;
import static org.apache.activemq.artemis.core.protocol.mqtt.MQTT2Util.MQTT_MESSAGE_RETAIN_KEY;
import static org.apache.activemq.artemis.core.protocol.mqtt.MQTT2Util.MQTT_PAYLOAD_FORMAT_INDICATOR_KEY;
import static org.apache.activemq.artemis.core.protocol.mqtt.MQTT2Util.MQTT_RESPONSE_TOPIC_KEY;
import static org.apache.activemq.artemis.core.protocol.mqtt.MQTT2Util.MQTT_USER_PROPERTY_EXISTS_KEY;
import static org.apache.activemq.artemis.core.protocol.mqtt.MQTT2Util.MQTT_USER_PROPERTY_KEY_PREFIX_SIMPLE;

/**
 * Handles MQTT Exactly Once (QoS level 2) Protocol.
 */
public class MQTT2PublishManager {
   ...
   private MQTT2Session session;

   private final Object lock = new Object();

   private MQTT2SessionState state;

   private MQTT2SessionState.OutboundStore outboundStore;

   private boolean closeMqttConnectionOnPublishAuthorizationFailure;

   public MQTT2PublishManager(MQTT2Session session, boolean closeMqttConnectionOnPublishAuthorizationFailure) {
   ...
   private SimpleString createManagementAddress() {
      return SimpleString.of(MQTT2Util.MANAGEMENT_QUEUE_PREFIX + session.getState().getClientId());
   }
   ...
   private void createManagementQueue() throws Exception {
      ...
      if (q == null) {
         session.getServer().createQueue(QueueConfiguration.of(managementAddress)
                                            ...
                                            .setDurable(MQTT2Util.DURABLE_MESSAGES));
      }
   }
   ...
   void sendToQueue(MqttPublishMessage message, boolean internal) throws Exception {
      synchronized (lock) {
         if (createProducer) {
            session.getServerSession().addProducer(senderName, MQTT2ProtocolManagerFactory.MQTT_PROTOCOL_NAME, ServerProducer.ANONYMOUS);
            createProducer = false;
         }
         String topic = message.variableHeader().topicName();
         if (session.getVersion() == MQTTVersion.MQTT_5) {
            Integer alias = MQTT2Util.getProperty(Integer.class, message.variableHeader().properties(), TOPIC_ALIAS);
            if (alias != null) {
         ...
         String coreAddress = MQTT2Util.getCoreAddressFromMqttTopic(topic, session.getWildcardConfiguration());
         SimpleString address = SimpleString.of(coreAddress, session.getCoreMessageObjectPools().getAddressStringSimpleStringPool());
         Message serverMessage = MQTT2Util.createServerMessageFromByteBuf(session, address, message);
         int qos = message.fixedHeader().qosLevel().value();
         if (qos > 0) {
            serverMessage.setDurable(MQTT2Util.DURABLE_MESSAGES);
         }
         ...
         if (qos < 2 || !qos2PublishAlreadyReceived) {
            ...
            try {
               ...
               MQTT2Logger.LOGGER.failedToPublishMqttMessage(t.getMessage(), t);
               tx.rollback();
               throw t;
            }
         } else if (qos2PublishAlreadyReceived) {
            MQTT2Logger.LOGGER.ignoringQoS2Publish(state.getClientId(), packetId);
         }
   ...
   void sendPubRelMessage(Message message) {
      int messageId = message.getIntProperty(MQTT2Util.MQTT_MESSAGE_ID_KEY);
   ...
   void handlePubRec(int messageId) throws Exception {
      try {
         ...
         if (ref != null) {
            Message m = MQTT2Util.createPubRelMessage(session, getManagementAddress(), messageId);
         ...
         MQTT2Logger.LOGGER.failedToAckMessage(session.getState().getClientId(), e);
      }
   }
   ...
   private boolean publishToClient(int messageId, ICoreMessage message, int deliveryCount, int qos, long consumerId) throws Exception {
      String topic = MQTT2Util.getMqttTopicFromCoreAddress(message.getAddress() == null ? "" : message.getAddress(), session.getWildcardConfiguration());
      ...
      int remainingLength = MQTT2Util.calculateRemainingLength(topic, mqttProperties, payload);
      ...
      if (session.getVersion() == MQTTVersion.MQTT_5 && maxSize != 0) {
         int size = MQTT2Util.calculateMessageSize(publish);
         if (size > maxSize) {
            /*
   ...
   private int decideQoS(Message message, ServerConsumer consumer) {
      ...
      int qos = 2;
      if (message.containsProperty(MQTT2Util.MQTT_QOS_LEVEL_KEY)) {
         qos = message.getIntProperty(MQTT2Util.MQTT_QOS_LEVEL_KEY);
      }
