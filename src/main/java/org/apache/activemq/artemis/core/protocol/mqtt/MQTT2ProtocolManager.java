...
public class MQTT2ProtocolManager extends AbstractProtocolManager<MqttMessage, MQTT2Interceptor, MQTT2Connection, MQTT2RoutingHandler> implements NotificationListener {
   ...
   private final List<MQTT2Interceptor> incomingInterceptors = new ArrayList<>();
   private final List<MQTT2Interceptor> outgoingInterceptors = new ArrayList<>();

   private int defaultMqttSessionExpiryInterval = -1;

   private int topicAliasMaximum = MQTT2Util.DEFAULT_TOPIC_ALIAS_MAX;

   private int receiveMaximum = MQTT2Util.DEFAULT_RECEIVE_MAXIMUM;

   private int serverKeepAlive = MQTT2Util.DEFAULT_SERVER_KEEP_ALIVE;

   private int maximumPacketSize = MQTT2Util.DEFAULT_MAXIMUM_PACKET_SIZE;

   private boolean closeMqttConnectionOnPublishAuthorizationFailure = true;

   private boolean allowLinkStealing = true;

   private final MQTT2RoutingHandler routingHandler;

   private MQTT2StateManager sessionStateManager;

   MQTT2ProtocolManager(ActiveMQServer server,
      ...
      routingHandler = new MQTT2RoutingHandler(server);
      sessionStateManager = MQTT2StateManager.getInstance(server);
      server.registerActivateCallback(new CleaningActivateCallback() {
         @Override
         public void deActivate() {
            MQTT2StateManager.removeInstance(server);
   ...
   public MQTT2ProtocolManager setDefaultMqttSessionExpiryInterval(int sessionExpiryInterval) {
   ...
   public MQTT2ProtocolManager setTopicAliasMaximum(int topicAliasMaximum) {
   ...
   public MQTT2ProtocolManager setReceiveMaximum(int receiveMaximum) {
   ...
   public MQTT2ProtocolManager setMaximumPacketSize(int maximumPacketSize) {
   ...
   public MQTT2ProtocolManager setServerKeepAlive(int serverKeepAlive) {
   ...
   public void onNotification(Notification notification) {
      ...
      if (protocolName == null || !protocolName.toString().startsWith(MQTT2ProtocolManagerFactory.MQTT_PROTOCOL_NAME))
         return;
      ...
      if (distance > 0) {
          ...
          */
         MQTT2Connection mqttConnection = sessionStateManager.getConnectedClients().get(clientId);
   ...
   public ProtocolManagerFactory getFactory() {
      return new MQTT2ProtocolManagerFactory();
   }
   ...
   public ConnectionEntry createConnectionEntry(Acceptor acceptorUsed, Connection connection) {
      try {
         MQTT2Connection mqttConnection = new MQTT2Connection(connection);
         /*
          ...
          */
         ConnectionEntry entry = new ConnectionEntry(mqttConnection, null, System.currentTimeMillis(), getServerKeepAlive() == -1 || getServerKeepAlive() == 0 ? -1 : getServerKeepAlive() * MQTT2Util.KEEP_ALIVE_ADJUSTMENT);

         NettyServerConnection nettyConnection = ((NettyServerConnection) connection);
         MQTT2ProtocolHandler protocolHandler = nettyConnection.getChannel().pipeline().get(MQTT2ProtocolHandler.class);
   ...
   public void addChannelHandlers(ChannelPipeline pipeline) {
       ...
       */
      pipeline.addLast(new MqttDecoder(MQTT2Util.MAX_PACKET_SIZE));

      pipeline.addLast(new MQTT2ProtocolHandler(server, this));
   }
   ...
   @Override
   public MQTT2RoutingHandler getRoutingHandler() {
      return routingHandler;
   }

   public String invokeIncoming(MqttMessage mqttMessage, MQTT2Connection connection) {
      return super.invokeInterceptors(this.incomingInterceptors, mqttMessage, connection);
   }

   public String invokeOutgoing(MqttMessage mqttMessage, MQTT2Connection connection) {
      return super.invokeInterceptors(this.outgoingInterceptors, mqttMessage, connection);
   }

   public MQTT2StateManager getStateManager() {
