 ...
 */
public class MQTT2ProtocolHandler extends ChannelInboundHandlerAdapter {
   ...
   public MQTT2ProtocolHandler(ActiveMQServer server, MQTT2ProtocolManager protocolManager) {
      this.server = server;
      this.protocolManager = protocolManager;
      this.mqttMessageActor = new Actor<>(server.getThreadPool(), this::act);
   }

   void setConnection(MQTT2Connection connection, ConnectionEntry entry) throws Exception {
      this.connectionEntry = entry;
      this.connection = connection;
      this.session = new MQTT2Session(this, connection, protocolManager, server.getConfiguration().getWildcardConfiguration());
   }
   ...
   public void channelRead(ChannelHandlerContext ctx, Object msg) {
      ...
      MQTT2Util.logMessage(session.getState(), message, true, session.getVersion());
   ...
   public void act(MqttMessage message) {
      try {
         ...
         MQTT2Logger.LOGGER.errorProcessingControlPacket(message.toString(), e);
         if (session.getVersion() == MQTTVersion.MQTT_5) {
            sendDisconnect(MQTTReasonCodes.IMPLEMENTATION_SPECIFIC_ERROR);
         }
         disconnect(true);
   ...
   void handleAuth(MqttMessage auth) throws Exception {
      byte[] authenticationData = MQTT2Util.getProperty(byte[].class, ((MqttReasonCodeAndPropertiesVariableHeader)auth.variableHeader()).properties(), AUTHENTICATION_DATA);
      String authenticationMethod = MQTT2Util.getProperty(String.class, ((MqttReasonCodeAndPropertiesVariableHeader)auth.variableHeader()).properties(), AUTHENTICATION_METHOD);
   ...
   void disconnect(boolean error, MqttMessage disconnect) {
      if (disconnect != null && disconnect.variableHeader() instanceof MqttReasonCodeAndPropertiesVariableHeader) {
         Integer sessionExpiryInterval = MQTT2Util.getProperty(Integer.class, ((MqttReasonCodeAndPropertiesVariableHeader)disconnect.variableHeader()).properties(), SESSION_EXPIRY_INTERVAL, null);
   ...
   void handlePublish(MqttPublishMessage message) throws Exception {
      if (session.getVersion() == MQTTVersion.MQTT_5 && session.getProtocolManager().getMaximumPacketSize() != -1 && MQTT2Util.calculateMessageSize(message) > session.getProtocolManager().getMaximumPacketSize()) {
   ...
   void handleSubscribe(MqttSubscribeMessage message) throws Exception {
      Integer subscriptionIdentifier = MQTT2Util.getProperty(Integer.class, message.idAndPropertiesVariableHeader().properties(), SUBSCRIPTION_IDENTIFIER, null);
   ...
   protected void sendToClient(MqttMessage message) {
      ...
      MQTT2Util.logMessage(session.getState(), message, false, session.getVersion());
   ...
   private void calculateKeepAlive(MqttConnectMessage connect) {
      ...
      if (serverKeepAlive == -1 || (clientKeepAlive <= serverKeepAlive && clientKeepAlive != 0)) {
         connectionEntry.ttl = clientKeepAlive * MQTT2Util.KEEP_ALIVE_ADJUSTMENT;
      } else {
   ...
   private LinkStealingResult handleLinkStealing() throws Exception {
      ...
      if (protocolManager.getStateManager().isClientConnected(clientID)) {
         MQTT2Connection existingConnection = protocolManager.getStateManager().getConnectedClient(clientID);
         if (protocolManager.isAllowLinkStealing()) {
            MQTT2Session existingSession = protocolManager.getStateManager().getSessionState(clientID).getSession();
